export default {
    ...
    groups: {
      ...
      form: {
        ...
        burstMode: '超刷开关',
        burstModeHint: '开启后保留缓存粘性，其他账号按 ID 升序集中承载请求；关闭时使用默认调度。',
        burstModeThreshold: '账号并发阈值',
        burstModeThresholdHint: '新请求会按账号 ID 顺序集中填充到该并发比例，剩余容量仅供已命中缓存粘性的请求使用。',
        burstModeThresholdRange: '账号并发阈值必须是 1 到 100 之间的整数',
        burstMode429RetryCount: '429 同账号重试次数',
        burstMode429RetryCountHint: '开启超刷后，单个账号收到 429 时最多连续重试的次数；默认 10 次。',
        burstMode429RetryCountRange: '429 重试次数必须是 1 到 100 之间的整数',
        burstModeHighUsage: '高用量集中模式',
        burstModeHighUsageHint: '仅适用于 OpenAI。开启后忽略缓存粘性，优先把请求集中到当前用量达到 95% 以上的账号；仍受上方并发阈值限制。',
      ...
      antiDegrade: {
        label: '统一防降智策略',
        hint: '给本分组所有账号套用同一档防降智预设，优先级高于账号自身的并发与指纹设置。选「关闭」则各组内账号用自己的设置；关闭时会先回滚由本分组设置过的账号。',
        off: '关闭（各账号用自己的设置）',
        offHint: '不干预组内账号：并发、指纹身份和 TLS 模板都按各账号自己的字段生效。',
        risk: '风险',
        syncTitle: '收敛状态',
        syncUnavailable: '本实例未启用收敛器，分组预设不会自动生效。',
        syncCounts: '成员 {total} · 已应用 {applied} · 已回滚 {reverted} · 跳过 {skipped} · 失败 {failed}',
        syncNow: '立即收敛一轮',
        syncTriggered: '已执行一轮收敛',
        syncFailed: '触发收敛失败',
        syncLastRun: '最近一轮：{at}',
        syncRunning: '进行中',
        syncFailures: '例：账号 {first} 失败 —— {reason}',
      },
      subscription: {
        title: '订阅设置',
        type: '计费类型',

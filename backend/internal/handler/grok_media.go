...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	for {
		...
		selectionCtx := service.WithBurstModeRetryAccount(requestCtx, burstRetryAccountID)
		...
		if boundLookupAccountID > 0 {
			selection, scheduleDecision, err = h.gatewayService.SelectGrokMediaVideoRequestAccount(
				selectionCtx, apiKey.GroupID, sessionHash, boundLookupAccountID, routingModel,
			)
		} else {
		...
		if err != nil {
			...
			if errors.As(err, &failoverErr) {
				...
				ok, retryLimit, retryDelay := burstSameAccountRetry(requestCtx, failoverErr, account, sameAccountRetryCount[account.ID])
				if ok {

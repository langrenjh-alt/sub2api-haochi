...
func (s *OpenAIGatewayService) forwardAsChatCompletions(
	...
	if account.Platform == PlatformGrok {
		// Prefer Responses for every losslessly bridgeable Grok request so the
		// fork's cache routing remains available to OAuth and API-key accounts,
		// including API-key accounts that target a custom xAI-compatible
		// endpoint which serves /responses. Preserve unsupported Chat Completions
		// shapes on the raw endpoint rather than silently dropping or rewriting
		// their fields.

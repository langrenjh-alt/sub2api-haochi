...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	budget := newOpenAISelectionProbeBudget()
	sharedAccounts, err := s.service.listSchedulableAccountRefs(ctx, req.GroupID, req.Platform)
	if err != nil {
	...
	// Official Grok quota gates operate on request-local values. Convert the
	// shared snapshot references once, then keep the filtered slice local.
	accounts := derefAccounts(sharedAccounts)
	...
	sharedAccounts = accountRefs(accounts)
	...
	filterStats := openAISelectionFilterStats{pool: len(sharedAccounts)}
	filtered := make([]*Account, 0, len(sharedAccounts))
	loadReq := make([]AccountWithConcurrency, 0, len(sharedAccounts))

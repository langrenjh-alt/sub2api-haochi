...
func (s *OpenAIGatewayService) forwardGrokChatCompletionsViaResponses(
	...
	// Grok CLI consumes the Chat-compatible top-level field. The generic
	// converter's reasoning.effort form is accepted but not applied.
	...
	if result != nil {
		...
		// Usage attribution follows the ingress contract, not the fact that the
		// Grok Responses upstream always uses SSE internally.

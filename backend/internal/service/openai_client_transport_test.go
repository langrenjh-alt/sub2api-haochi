...
func TestResolveOpenAIWSDecisionByClientTransport(t *testing.T) {
	...
	require.Equal(t, OpenAIUpstreamTransportResponsesWebsocketV2, httpDecision.Transport)
	require.Equal(t, "ws_v2_enabled_http_ingress", httpDecision.Reason)
	...
	httpKeepHTTP := resolveOpenAIWSDecisionByClientTransport(openAIWSHTTPDecision("account_mode_off"), OpenAIClientTransportHTTP)
	require.Equal(t, OpenAIUpstreamTransportHTTPSSE, httpKeepHTTP.Transport)
	require.Equal(t, "account_mode_off", httpKeepHTTP.Reason)
}
